...
function cleanJsonResponse(buffer) {
...
export class Client extends EventEmitter {
    ...
    async listCollections() {
        ...
        if (cached && cached.matches) {
            return cached.matches; // Type assertion for collections list
        }
        const response = await this.request({
            method: 'GET',
            path: '/collections'
     ...
     */
    async getCollectionDimensions(collectionName) {
     ...
     */
    async validateVectorDimensions(collectionName, vector) {
     ...
     */
    async batchInsert(records, collectionName = 'default', options = {}) {
        if (!records.length) {
            return { upserted_count: 0 };
        }
        const { validateDimensions = true, parallelism = Math.min(cpus().length, 8) } = options;
            ...
            if (expectedDims !== undefined) {
                for (let i = 0; i < Math.min(records.length, 10); i++) {
                    ...
                    if (vector.length !== expectedDims) {
                        throw new Error(`Vector dimension mismatch in record ${i}: got ${vector.length}, expected ${expectedDims} for collection '${collectionName}'`);
                    }
                }
            }
        }
     ...
     */
    vectorOperations(collectionName = 'default') {
        return new VectorOperations(this.host, collectionName, {
     ...
     */
    vector_operations(collectionName = 'default') {
    ...
    async performRequest(options) {
        ...
        return this.performHttp1Request(options, isHttps);
    }
    /**
     * HTTP/1.1 fallback request
     */
    async performHttp1Request(options, isHttps) {
     ...
     */
    async getOrCreateHttp2Connection() {
        const url = new URL(this.host);
 ...
 */
export function createClient(host, config = {}) {
