...
interface VectorOpsContext {
	request: <T = any>(options: any) => Promise<T>;
	queryCache: QueryCache;
	bufferPool: BufferPool;
	logger: Logger;
	collectionCache: Map<string, CollectionInfo>;
	getCollectionDimensions: (collectionName: string) => Promise<number | undefined>;
	validateVectorDimensions: (collectionName: string, vector: number[]) => Promise<boolean>;
}
...
interface QueryOptions extends SearchParams {
	skipCache?: boolean;
	validateDimensions?: boolean;
}
...
export class VectorOperations {
	...
	private readonly context: VectorOpsContext;
	...
	private upsertQueue: VectorRecord[] = [];
	private upsertTimeout?: NodeJS.Timeout;
	private readonly batchDelay = 50; // 50ms batch delay for auto-batching

	constructor(
		host: string,
		collectionName: string,
		context: VectorOpsContext
	) {
	...
	async upsert(
		...
		const {
			batchSize = 5000,
			showProgress = false,
			parallelWorkers = Math.min(this.maxWorkers, 8),
	 ...
	 */
	async upsertBatch(batch: VectorRecord[]): Promise<UpsertResponse> {
		...
		try {
			const response = await this.context.request<UpsertResponse>({
				method: 'POST',
				path: '/vectors/upsert',
	 ...
	 */
	async query(params: QueryOptions = {}): Promise<SearchResults> {
		const {
			vector,
			topK = 10,
			includeValues = false,
		...
		if (validateDimensions) {
			...
			if (!isValid) {
				...
				throw new Error(
					`Query vector dimension mismatch: got ${vector.length}, expected ${expectedDims} for collection '${this.collectionName}'`
				);
			}
		}
		...
		if (!skipCache) {
			cacheKey = this.context.queryCache.computeCacheKey(
				this.collectionName,
				vector,
				topK,
				{
		...
		try {
			const response = await this.context.request<QueryResponse>({
				method: 'POST',
				path: '/vectors/query',
			...
			const result: SearchResults = { matches };
	...
	async batchQuery(
			...
			if (expectedDims !== undefined) {
				for (let i = 0; i < Math.min(vectors.length, 5); i++) {
					if (vectors[i].length !== expectedDims) {
						throw new Error(
							`Query vector ${i} dimension mismatch: got ${vectors[i].length}, expected ${expectedDims} for collection '${this.collectionName}'`
						);
					}
				}
			}
		...
		if (vectors.length > 10 && this.workerPool.isHealthy()) {
			try {
				...
				processedVectors = await Promise.all(
					vectors.map(vec => Promise.resolve(this.optimizeVector(vec)))
				);
			}
		} else {
		...
		try {
			const response = await this.context.request<BatchQueryResponse>({
				method: 'POST',
	...
	async delete(ids: string[]): Promise<DeleteResponse> {
		...
		try {
			const response = await this.context.request<DeleteResponse>({
				method: 'POST',
				path: '/vectors/delete',
				body: JSON.stringify(payload),
				timeout: 30
	 ...
	 */
	async getVector(vectorId: string): Promise<VectorRecord | null> {
		try {
			const response = await this.context.request<VectorRecord>({
				method: 'GET',
	...
	async countVectors(): Promise<number> {
		try {
			const collectionInfo = await this.context.request<CollectionInfo>({
				method: 'GET',
	 ...
	 */
	async getCollectionDimensions(): Promise<number | undefined> {
	 ...
	 */
	async validateVectorDimensions(vector: number[]): Promise<boolean> {
	 ...
	 */
	private async validateBatchDimensions(vectors: VectorRecord[]): Promise<void> {
		const expectedDims = await this.context.getCollectionDimensions(this.collectionName);
		if (expectedDims === undefined) return;
	 ...
	 */
	private async preprocessVectorsParallel(vectors: VectorRecord[], workerCount: number): Promise<VectorRecord[]> {
		...
		try {
			...
			const processedChunks = await Promise.all(
				chunks.map(chunk =>
					this.workerPool.execute('preprocessVectors', {
						vectors: chunk,
						collectionName: this.collectionName
					})
				)
	 ...
	 */
	private preprocessVectorsDirect(vectors: VectorRecord[]): VectorRecord[] {
	 ...
	 */
	private optimizeVector(vector: number[]): number[] {
...
class WorkerPool {
	...
	constructor(private maxWorkers: number, logger: Logger) {
		this.logger = logger;
	...
	private executeMainThread<T>(task: string, data: any): Promise<T> {
	...
	private preprocessVectorsMainThread(vectors: any[]): any[] {
		return vectors.map(vec => {
			const vectorValues = vec.vector || vec.values;
			if (!vectorValues) {
				throw new Error(`Vector missing 'values' or 'vector' field for ID: ${vec.id}`);
	...
	private processQueryVectorsMainThread(vectors: number[][]): number[][] {
		return vectors.map(vec => this.optimizeVectorMainThread(vec));
	}

	private processBatchMainThread(batch: any[]): any[] {
	...
	private optimizeVectorMainThread(vector: number[]): number[] {
		return vector.map(v => {
			const num = Number(v);
			return Math.round(num * 1000000) / 1000000;
		});
	...
	getStats(): { totalWorkers: number; availableWorkers: number; queueLength: number } {
		return {
